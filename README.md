...
> **⚠️ 这个 exe 可能被 Windows Defender 误报拦截**
>
> 它用 PyInstaller 单文件打包，且**尚未做代码签名**，因此会被 Defender 的机器学习启发式判成
> `Trojan:Win32/Sabsik.FL.A!ml`。
>
> 注意 `!ml` 这个后缀 —— 它表示**启发式/机器学习判定，并非匹配到了真实的病毒特征**。
> 这是 PyInstaller `--onefile` 长期存在的已知误报，与代码本身无关。本项目全部源码公开，
> 你随时可以按[方式二](#方式二从源码运行)自己跑一遍再打包。
>
> 被拦时的处理办法见下方[「常见问题」](#常见问题)；按里面的步骤仍然装不上，欢迎开 issue。
...
**下载 exe 被拦，或者下载完文件就消失了？**

这是 Defender 对「PyInstaller 单文件 + 未签名」的误报（`Trojan:Win32/Sabsik.FL.A!ml`），
详见[方式一](#方式一下载现成的-exe)里的说明。**不是真的病毒**，处理办法：

1. **文件下载后消失** → Defender 把它隔离了。到「Windows 安全中心 → 病毒和威胁防护 →
   保护历史记录」，找到那条 `Trojan:Win32/Sabsik.FL.A!ml`，点「操作 → 还原」。
2. **还原不了（显示已删除）** → 先加排除项再重新下载：
   「病毒和威胁防护 → 管理设置 → 排除项 → 添加或删除排除项 → 添加 → 文件夹」，
   选中你打算放 `dsh-tray.exe` 的目录（或先建一个空目录专门放它）。
3. **Edge 提示「已阻止不安全的下载」** → 在下载提示上点 `…` → 选「保留」。
4. **不想折腾杀软设置** → 走[方式二](#方式二从源码运行)，源码全在仓库里，自己跑。

> 顺带一提：如果只是弹「Windows 已保护你的电脑 / 未知发布者」，那只是 SmartScreen 对
> 「从 Internet 下载且无签名」文件的例行提示，可以勾选「更多信息 → 仍要运行」。
> 也可以用 `Unblock-File .\dsh-tray.exe` 去掉文件的「来自 Internet」标记 ——
> 但这**只影响 SmartScreen 提示，不能解除 Defender 的病毒判定**。
