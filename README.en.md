...
<p align="center">
  <img src="assets/tray-menu-en.png" alt="Delfin tray menu (service running)">
  <br>
  <sub>The menu reflects live service state — "Stop / Restart" grey out while the service is down, and "Check for updates" reports its result inline.</sub>
</p>
...
>
> That is **SmartScreen's reputation prompt, not a malware verdict** — the exe is
> **not code signed**, so Windows cannot verify who published it and blocks freshly
> downloaded programs by default. **It is not a virus.**
>
> Allow it either way:
> 1. Click **"More info" → "Run anyway"**
> 2. Right-click the exe → **Properties** → tick **"Unblock"** at the bottom (no prompt after that)
>
> If Edge says "This download was blocked", choose **"Keep"**.
>
> > Earlier this exe was also caught by Defender's heuristic as `Trojan:Win32/Sabsik.FL.A!ml`
> > and **quarantined outright**. That false positive was reported to Microsoft and has been
> > **cleared**. If you still get an outright deletion, your local signature definitions are
> > probably stale — check for updates in Windows Security first, and open an issue if it persists.
>
> All sources are public, so you can build and run it yourself via
> [Option 2](#option-2-run-from-source). For details see the [FAQ](#faq).
...
2. **Right-click the exe → Properties → tick "Unblock"** — strips the mark-of-the-web, no prompt
   afterwards. Command-line equivalent: `Unblock-File .\dsh-tray.exe`

**Edge says "This download was blocked"?** → click `…` on the download item and choose **Keep**.

**The file vanished right after downloading? (now rare)**

Earlier this exe was caught by Defender's machine-learning heuristic as
`Trojan:Win32/Sabsik.FL.A!ml` and quarantined outright (the `!ml` suffix means a heuristic
verdict, **not a match against real malware signatures**). That false positive has been
**reported to Microsoft and cleared.**

If you still get an outright deletion, your local signature definitions are most likely stale:
update them first under Windows Security → Virus & threat protection → Virus & threat
protection updates, then download again. If that does not help, add an exclusion temporarily:

1. Virus & threat protection → Manage settings → Exclusions → Add or remove exclusions → Add →
   Folder, and pick the folder you intend to keep `dsh-tray.exe` in.
2. To recover an already-quarantined copy: Protection history → find the entry → Actions → Restore.

3. **Rather not touch antivirus settings at all?** → use
   [Option 2](#option-2-run-from-source) and run it from source instead — all the code is here.
