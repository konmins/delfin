...
# ---------- 启动器（dsh-runner.cmd） ----------
# 内嵌一份启动器源码。发布包是 exe + cmd 两个文件，用户只拿走 exe、或者在
# build 输出目录（dist\）里直接双击 exe 调试时，点「启动」只会弹一句
# 「未找到 dsh-runner.cmd」然后毫无反应 —— 托盘看起来完全像是坏的。
# 同目录缺失就用这份内容补齐；已存在的文件一律不动（那可能是用户改过的）。
#
# 内容需与仓库根的 dsh-runner.cmd 保持同步（selftest.py 有断言）。
RUNNER_TEXT = r"""@echo off
rem DeepSeek Harness runner - launched by dsh-tray.exe
rem Auto-generated by dsh-tray.exe because this file was missing.
...
def ensure_runner():
    """返回可用的启动器路径；同目录缺失时用内嵌内容生成一份

    生成失败（目录只读、无权限等）返回 None，由调用方给出可见的错误提示。
    """
    if os.path.exists(RUNNER):
        return RUNNER
    try:
        # .cmd 必须是 CRLF 且不带 BOM：cmd.exe 会把 BOM 当成第一条命令的一部分
        with open(RUNNER, "w", encoding="ascii", newline="\r\n") as f:
            f.write(RUNNER_TEXT)
    except OSError as e:
        logging.error("无法生成启动器 %s: %s", RUNNER, e)
        return None
    logging.info("同目录缺少 dsh-runner.cmd，已自动生成: %s", RUNNER)
...
def start_service():
    ...
    try:
        ...
        if not runner:
            icon.notify(t("runner_missing", APP_DIR), "DeepSeek Harness")
            return False
